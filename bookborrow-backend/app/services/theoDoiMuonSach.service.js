...
class TheoDoiMuonSachService {
  ...
  // // Đếm số lượng bản ghi trong khoảng thời gian
  // // Đếm số lượng bản ghi trong khoảng thời gian
  // async countByPeriod(startOfPeriod, endOfPeriod) {
  //   try {
  //     const count = await this.TheoDoiMuonSach.countDocuments({
  //       NgayMuon: { $gte: startOfPeriod, $lte: endOfPeriod },
  //     });
  //     return count;
  //   } catch (error) {
  //     throw new Error("Error counting records by period");
  //   }
  // }
  ...
  async countByDay(date) {
    ...
    const result = await this.TheoDoiMuonSach.countDocuments({
      NgayMuon: { $gte: startOfDay, $lte: endOfDay },
    });

    return result;
  }

  async countByWeek(date) {
    const startOfWeek = getStartOfWeek(date);
    const endOfWeek = getEndOfWeek(date);

    const result = await this.TheoDoiMuonSach.countDocuments({
      NgayMuon: { $gte: startOfWeek, $lte: endOfWeek },
    });

    return result;
  }

  async countByMonth(date) {
    const startOfMonth = new Date(date.getFullYear(), date.getMonth(), 1);
    const endOfMonth = new Date(date.getFullYear(), date.getMonth() + 1, 0);

    const result = await this.TheoDoiMuonSach.countDocuments({
      NgayMuon: { $gte: startOfMonth, $lte: endOfMonth },
    });

    return result;
  }

  async countByYear(date) {
    const startOfYear = new Date(date.getFullYear(), 0, 1);
    const endOfYear = new Date(date.getFullYear(), 11, 31);

    const result = await this.TheoDoiMuonSach.countDocuments({
      NgayMuon: { $gte: startOfYear, $lte: endOfYear },
    });
